...
<script type="text/javascript">
	...
	// 전체 회원 목록 함수 + page 전역변수 선언이 필요함
	var page = 1; // 페이지의 시작은 무조건 1페이지부터
	...
	function fnPrintMemberList(map){
		// 페이지 처리 모든 정보를 변수 p로 저장
		var p = map.pageUtils;		// totalRecord에서 사용.
		$('#member_list').empty();
		if (p.totalRecord == 0) {
			$('<tr>')
	...
	} // end fnPrintMemberList(map)

	
	// 페이징 출력 함수 
	function fnPaging(p){	// pageUtils = p
		// 페이징 만들기
		$('#paging').empty();
		// 1페이지로 이동
		if(page == 1){
			$('<div class="disable_link">&lt;&lt;</div>').appendTo('#paging');
			// $('<div>').addClass('disable_link').html('&lt;&lt').appendTo('#paging')	// entity코드는 html로 넣어야함.
		} else{
			$('<div class="enable_link" data-page="1">&lt;&lt;</div>').appendTo('#paging');
			// $('<div>').addClass('enable_link').html('&lt;&lt').attr('data-page', 1).appendTo()
		}
		// 이전 블록으로 이동
		...
		// 마지막 페이지로 이동
		if(page == p.totalPage){
			$('<div class="disable_link">&gt;&gt</div>').appendTo('#paging');			
		} else{
			$('<div class="enable_link" data-page="'+ p.totalPage +'">&gt;&gt</div>').appendTo('#paging');			
		}
		
	}// end fnPrintPaging
	
	// 페이징 링크 처리 함수. (전역변수 page의 값을 바꾸고 fnFindAllMember() 호출이 목적이다. / 전역변수 page가 변환하면 fnFindAllMember에서 보내는 전역변수 page의 값도 변경된 값으로 들어간다.)
	...
	function fnAddMember() {
		$('#insert_btn').click(function(){
			let member = JSON.stringify({
				id: $('#id').val(),
				name: $('#name').val(),
				gender: $('input:radio[name="gender"]:checked').val(), // css에서 클래스를 불러올 때 사용하는 문법을 참고. checked된 속성의 value를 불러오겠다.
				address: $('#address').val()
			});
			$.ajax({
	...
	function fnFindMember(){
		$('body').on('click', '.view_btn', function(){
			$.ajax({
				url: '/jenkinsproject/api/members/' + $(this).prev().val(),	// url에 데이터를 포함해서 보냄, 기존 파라미터 추가로 보내는 방식과는 차이가 있다.
				type: 'get',
				dataType: 'json',
				success: function(map){
					if(map.member != null){
						...
						alert($(this).prev().val() + '번 회원의 정보가 없습니다.');
					}
				},
				error: function(){
